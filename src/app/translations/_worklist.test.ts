...
/** THROWAWAY. Regenerates the translation worklist as JSON. Delete before commit. */
...
it("writes the worklist", async () => {
  ...
  collect(puzzles, authored);
  collect(TEST_ITEMS, authored);
  collect(TAGS, authored);
  ...
  const locales = Object.keys(worklist);
  // eslint-disable-next-line no-console
  console.log(
    `locales ${locales.join(",")} count ${worklist[locales[0]]?.length ?? 0}`,
  );
});
