...
> **Status: alpha.** Playable end to end and deployed, but the deck, the
> engine's data shapes and the review bank are all still growing week to week,
> and nothing here is API-stable. Puzzles get added, shapes get added, and the
> schema gains union members as new lessons need them. Treat pinned versions,
> URLs and the puzzle schema as liable to change. Bug reports and puzzle
...
This repository is the **Phase 0 prototype**: 58 fully playable puzzles on a generic engine, served one-per-day Wordle-style, in ten languages, plus a **spaced-repetition review** built on an item bank of 794 scenarios, for testing whether you can spot a flaw when nobody has told you one is there. Learning a puzzle schedules its skill; the review then comes due over time and draws a fresh scenario each visit, a real trap for that skill or a genuinely sound decoy, and asks you to tell them apart. Signing in makes that schedule follow you between devices.

The bank is sized for spaced repetition rather than for a quiz: at least ten distinct scenarios per skill, so a learner climbing the eight-stage review ladder never meets the same one twice, and 205 of the 794 are cases where the reasoning is **genuinely sound**. Without those, the winning strategy would be to answer "trap" every time and the score would measure nothing.

The full list of shipped lessons lives in [`src/puzzles/index.ts`](./src/puzzles/index.ts), which is the registry the app reads, so it cannot drift from what actually ships. It runs from Simpson's paradox and the base-rate fallacy through survivorship bias, lead-time bias and immortal time bias to Campbell's law, map-projection distortion, police-versus-survey crime counts and the margin of error on a poll lead.
...
Before accounts go live the privacy page needs a contact address, which is a build variable rather than a committed line: set it once in the Cloudflare dashboard, or for local builds copy `.env.example` to a gitignored `.env.local` and fill in `CONTACT_EMAIL` (a shell variable of the same name overrides it). Left unset the build warns on every run, and an address that looks like an example fails the build outright.
