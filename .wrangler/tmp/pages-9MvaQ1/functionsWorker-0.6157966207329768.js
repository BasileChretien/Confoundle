...
async function onRequestPost(ctx) {
  ...
  if (!Number.isFinite(day) || !Number.isFinite(score) || score < MIN_SCORE || score > MAX_SCORE) {
    return json({ error: "bad input" }, 400);
  }
  const key = `day:${day}`;
  const raw = await ctx.env.SCORES.get(key);
  const histogram = raw ? JSON.parse(raw) : {};
...
// ../.wrangler/tmp/pages-9MvaQ1/functionsRoutes-0.8133915014970252.mjs
var routes = [
  {
    routePath: "/api/score",
    mountPath: "/api",
...
// ../../../Users/Basile/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// ../../../Users/Basile/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
